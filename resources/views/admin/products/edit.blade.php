...
@section('content')
    <form method="POST" id="productForm" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <div class="row mt-4">
                        ...
                        <div class="col-md-6">
                            <div class="form-floating mb-3">
                                <input type="text" id="cost" class="form-control @error('cost') is-invalid @enderror"
                                       name="cost" value="{{ old('cost', \Illuminate\Support\Number::currency($product->getCost(), 'RUB', 'ru')) }}" placeholder="Стоимость продукта" required>
